    ...
    + UnsafeRandomAccessByRef<2, Item = Self::Item>>(source_arr: &mut Array<Self::Item, ArrayImplMut, 2>, indices: ExtInsType) -> RlstResult<Self>{
        ...
        match indices {
            ExtInsType::Axis(inds, axis, exchange_axis) => {
                ...
                if axis == 0{
                    let target_arr: DynamicArray<Self::Item, 2> = get_rows(inds, source_arr, exchange_axis);
                    Ok(Self{ext: target_arr})
                }
                else{
                    let target_arr: DynamicArray<Self::Item, 2> =  get_cols(inds, source_arr, exchange_axis);
                    Ok(Self{ext: target_arr})
