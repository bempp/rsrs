                ...
                match self.op_type{
                    OpType::Row(arr) => {
                        if inv 
                        {
                            ...
                            row_ops(self.col_indices, self.row_indices, self.dim, arr, right_arr, beta, trans)
                        }
                        else{
                            let beta: Self::Item = 1.0.into();
                            row_ops(self.col_indices, self.row_indices, self.dim, arr, right_arr, beta, trans)   
...
+ RawAccessMut<Item = Item>>(el_mat: ElementaryMatrix<Item>, mut right_arr: Array<Item, ArrayImpl, 2>, alpha: Item){
    ...
    let dim = el_mat.dim;
    let row_indices = el_mat.row_indices;
    for col in 0..dim{
...
+ RawAccessMut<Item = Item>>(el_mat: ElementaryMatrix<Item>, mut right_arr: Array<Item, ArrayImpl, 2>, trans: bool){
    ...
    if trans{
        col_indices = el_mat.row_indices;
        row_indices = el_mat.col_indices;
    }
    else{
        col_indices = el_mat.col_indices;
        row_indices = el_mat.row_indices;
    }
