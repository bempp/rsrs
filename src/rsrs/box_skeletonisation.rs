...
pub trait SkelBox<T: RlstScalar>{
    ...
    fn null_sketch_near_field(&self, ff_sketch: &mut Array<Self::Item, Self::ArrayImpl, 2>, target_inds: &mut Vec<usize>, sketch: &mut Array<Self::Item, Self::ArrayImpl, 2>, test: &mut Array<Self::Item, Self::ArrayImpl, 2>, tol_null: <Self::Item as RlstScalar>::Real);
    fn null_near_field(&mut self, target_inds: &mut Vec<usize>, far_field_sketch: &mut Array<Self::Item, Self::ArrayImpl, 2>, y_data: &mut BoxesData<Self::Item>, z_data: &mut BoxesData<Self::Item>, tol_null: <Self::Item as RlstScalar>::Real);  
    fn get_id_factor(&mut self, dim: usize, target_inds: &mut Vec<usize>, far_field_sketch: Array<Self::Item, Self::ArrayImpl, 2>, tol_id: <Self::Item as RlstScalar>::Real)-> Option<(ElementaryMatrix<Self::Item>, ElementaryMatrix<Self::Item>, Vec<usize>)>;
    ...
    Standard: Distribution<T::Real>,
{
    ...
    fn new(near_field_inds: Vec<usize>)->Self{
        ...
        Self{near_field_inds, ind_r, ind_s, ind_t}
    }
    ...
    fn get_id_factor(&mut self, dim: usize, target_inds: &mut Vec<usize>, far_field_sketch: Array<Self::Item, Self::ArrayImpl, 2>, tol_id: <Self::Item as RlstScalar>::Real)-> Option<(ElementaryMatrix<Self::Item>, ElementaryMatrix<Self::Item>, Vec<usize>)>{
        ...
        if id_sketch.rank < max_rank{
            ...
            self.ind_r.append(&mut target_inds[k..].to_vec());
            self.ind_s.append(&mut target_inds[0..k].to_vec());
    ...
    fn get_lu_factors(&mut self, y_data: &mut BoxesData<Self::Item>, z_data: &mut BoxesData<Self::Item>, tol_lstq: <Self::Item as RlstScalar>::Real)-> (ElementaryMatrix<Self::Item>, ElementaryMatrix<Self::Item>){
        ...
        let l_el_matrix: ElementaryMatrix<Self::Item> = <ElementaryMatrix<Self::Item> as ElementaryOperations>::new(y_data.dim, self.ind_t.clone(), self.ind_r.clone(), OpType::Row(l_fact), false).unwrap();
        let u_el_matrix: ElementaryMatrix<Self::Item> = <ElementaryMatrix<Self::Item> as ElementaryOperations>::new(y_data.dim, self.ind_r.clone(), self.ind_t.clone(), OpType::Row(u_fact), false).unwrap();
        (l_el_matrix, u_el_matrix)
    }

    fn decouple(&mut self, target_inds: &mut Vec<usize>, y_data: &mut BoxesData<Self::Item>, z_data: &mut BoxesData<Self::Item>, tols: &Tols<Self::Item>)->Rank<Self::Item>{//, level_indexing: &mut TreeData<'_, C>)->Rank<Self::Item>{
        let mut far_field_sketch: DynamicArray<Self::Item, 2> = empty_array();
        let start: Instant = Instant::now();
        self.null_near_field(target_inds, &mut far_field_sketch, y_data, z_data, tols.null);
        let duration: Duration = start.elapsed();
        println!("Nullification in {} ms", duration.as_millis());

        let mut aux_target_inds = target_inds.clone();
        let start: Instant = Instant::now();
        let id_factor: Option<(ElementaryMatrix<T>, ElementaryMatrix<T>, Vec<usize>)> = self.get_id_factor(y_data.dim, &mut aux_target_inds, far_field_sketch, tols.id);
        let duration: Duration = start.elapsed();
        ...
        match id_factor{
            Some((e_fact, f_fact, perm))=>{
                if !self.ind_r.is_empty(){
                    ...
                    let start: Instant = Instant::now();
                    let (l_fact, u_fact) = self.get_lu_factors(y_data, z_data, tols.lstq);
                    let duration: Duration = start.elapsed();
