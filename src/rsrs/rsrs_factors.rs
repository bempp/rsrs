...
pub trait IdFactorOperations: Sized {
    ...
    fn new(target_inds: &mut Vec<usize>, near_field_inds: &mut Vec<usize>, target_arr: DynamicArray<Self::Item, 2>, tol_id: <Self::Item as RlstScalar>::Real)-> Option<RlstResult<Self>>;

    fn mul<ArrayImplMut: UnsafeRandomAccessByValue<2, Item = Self::Item>
    + Shape<2>
    + RawAccessMut<Item = Self::Item>
    + UnsafeRandomAccessMut<2, Item = Self::Item>
    + UnsafeRandomAccessByRef<2, Item = Self::Item>>(&self, target_arr: &mut Array<Self::Item, ArrayImplMut, 2>, options: &FactorOptions, factor_type: IdFactorType, operation_type: DecFactorOpType);
}
...
impl <T:RlstScalar + MatrixInverse + MatrixId>IdFactorOperations for IdFactor<T>
{
    ...
    fn new(target_inds: &mut Vec<usize>, near_field_inds: &mut Vec<usize>, target_arr: DynamicArray<Self::Item, 2>, tol_id: <Self::Item as RlstScalar>::Real)-> Option<RlstResult<Self>>{
        ...
        if id_sketch.rank < max_rank{
            ...
            Some(Ok(Self{data: id_sketch.id_mat, perm: id_sketch.perm, ind_r, ind_s}))
        }
        else{
            None
        }
...
fn near_box_extraction<Item: RlstScalar + MatrixPseudoInverse>(ind_r: &mut [usize], near_field_inds: &mut [usize], sketch_data: &mut BoxesData<Item>, tol_lstq: <Item as RlstScalar>::Real, r_numbering: &Vec<usize>, t_numbering: &Vec<usize>)->(DynamicArray<Item, 2>, DynamicArray<Item, 2>, (Duration, Duration)){
...
pub trait LuFactorOperations: Sized {
    ...
    fn new(ind_r: &mut [usize], near_field_inds: &mut [usize], y_data: &mut BoxesData<Self::Item>, z_data: &mut BoxesData<Self::Item>, tol_lstq: <Self::Item as RlstScalar>::Real, hermitian: bool)-> RlstResult<Self>;
    
    fn mul<ArrayImplMut: UnsafeRandomAccessByValue<2, Item = Self::Item>
    + Shape<2>
    + RawAccessMut<Item = Self::Item>
    + UnsafeRandomAccessMut<2, Item = Self::Item>
    + UnsafeRandomAccessByRef<2, Item = Self::Item>>(&self, target_arr: &mut Array<Self::Item, ArrayImplMut, 2>, options: &FactorOptions, factor_type: LuFactorType, operation_type: DecFactorOpType);
...
impl <T:RlstScalar + MatrixInverse + MatrixPseudoInverse>LuFactorOperations for LuFactor<T>
{
    ...
    fn new(ind_r: &mut [usize], near_field_inds: &mut [usize], y_data: &mut BoxesData<Self::Item>, z_data: &mut BoxesData<Self::Item>, tol_lstq: <Self::Item as RlstScalar>::Real, hermitian: bool)-> RlstResult<Self>{
        ...
        let l_arr: DynamicArray<Self::Item, 2> = empty_array().simple_mult_into_resize(z_n.view(), z_r.view());
        ...
        if !hermitian{
            ...
            u_arr.view_mut().simple_mult_into_resize(y_r.view(), y_n.view());
            lu_ext_time = y_lu_ext_time + z_lu_ext_time;
            lu_solving_time = y_lu_solving_time + z_lu_solving_time;
        }
        else{
            lu_ext_time = z_lu_ext_time;
            lu_solving_time = z_lu_solving_time;
        }
        ...
        Ok(Self{l_arr, u_arr, ind_r: ind_r.to_vec(), ind_t})
    ...
    + UnsafeRandomAccessByRef<2, Item = Self::Item>>(&self, target_arr: &mut Array<Self::Item, ArrayImplMut, 2>, options: &FactorOptions, factor_type: LuFactorType, operation_type: DecFactorOpType){
        ...
        if self.u_arr.shape()[0]==0{
            match factor_type{
                LuFactorType::L => {},
                LuFactorType::U => {trans = !trans;},
            }

            match operation_type {
                DecFactorOpType::Left => {row_ops(self.ind_r.clone(), self.ind_t.clone(), &self.l_arr, target_arr, beta, trans);},
                DecFactorOpType::Right => {col_ops(self.ind_t.clone(), self.ind_r.clone(), &self.l_arr, target_arr, beta, trans);},
            }
        }
        else{
            match factor_type{
                LuFactorType::L => {
                    match operation_type {
                        DecFactorOpType::Left => {row_ops(self.ind_r.clone(), self.ind_t.clone(), &self.l_arr, target_arr, beta, trans);},
                        DecFactorOpType::Right => {col_ops( self.ind_r.clone(), self.ind_t.clone(), &self.l_arr, target_arr, beta, trans);},
                    }
                },
                LuFactorType::U => {
                    match operation_type {
...
pub struct DiagBoxFactor<T:RlstScalar>{
    diag_boxes: Vec<DynamicArray<T, 2>>
}
...
pub trait RsrsFactorsMul: Sized {
    type Item: RlstScalar;
    ...
    fn diag_errors(&self, arr: &mut DynamicArray<Self::Item, 2>);

    fn box_errors(&self, factor_num: usize, arr: &mut DynamicArray<Self::Item, 2>, near_field_inds: &[usize])->Errors<Self::Item>;

    fn get_boxes_errors(&self, arr: &mut DynamicArray<Self::Item, 2>)->VecErrors<<Self::Item as RlstScalar>::Real>;
}
...
impl <T:RlstScalar + MatrixInverse + MatrixId + MatrixPseudoInverse>RsrsFactorsMul for RsrsFactors<T>
{
    type Item = T;
    ...
    fn new() -> RlstResult<Self> {
        ...
        Ok(Self{dec_factors, perm_factor, diag_box_factor})
    }
    ...
    + UnsafeRandomAccessMut<2, Item = Self::Item>
    + UnsafeRandomAccessByRef<2, Item = Self::Item>>(&self, target_arr: &mut Array<Self::Item, ArrayImplMut, 2>, dec_factors: &DecFactors<Self::Item>, factor_options: &FactorOptions){
        dec_factors.id_factor.mul(target_arr, factor_options, IdFactorType::E, DecFactorOpType::Left);
        dec_factors.id_factor.mul(target_arr, factor_options, IdFactorType::F, DecFactorOpType::Right);
        dec_factors.lu_factor.mul(target_arr, factor_options, LuFactorType::L, DecFactorOpType::Left);
        dec_factors.lu_factor.mul(target_arr, factor_options, LuFactorType::U, DecFactorOpType::Left);
    }
    ...
    fn set_diagonal_boxes(&self, target_arr: &mut DynamicArray<Self::Item, 2>){
        ...
        self.perm_factor.right_mul(target_arr, &FactorOptions{ inv: false, trans: false});
    ...
    fn diag_errors(&self, arr: &mut DynamicArray<Self::Item, 2>){
        ...
        for diag_box in self.diag_box_factor.diag_boxes.iter(){
            ...
            diag_ae.push(res.norm_2_alloc().unwrap());
            count+=len_box;
        }
    }
    ...
    fn get_boxes_errors(&self, target_arr: &mut DynamicArray<Self::Item, 2>)->VecErrors<<Self::Item as RlstScalar>::Real>
    {
        ...
        for (box_num, dec_factors) in self.dec_factors.iter().enumerate(){
            let (arr_rf, arr_fr, arr_rt, arr_tr) = self.box_errors(box_num, target_arr, &dec_factors.near_field_inds);
            self.box_dec(target_arr, dec_factors, &factor_options);
            let (arr_rf_ae, arr_fr_ae, arr_rt_ae, arr_tr_ae) = self.box_errors(box_num, target_arr, &dec_factors.near_field_inds);
