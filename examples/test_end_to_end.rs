...
fn get_example(geometry_fn: fn(usize, &SimpleCommunicator) -> Vec<bempp_octree::Point>){
    ...
    let tols: Tols<f64> = Tols{id: 1e-5, null: 1e-15, lstq: 1e-15};
    //Create a low rank matrix
    let kernel_mat: Array<f64, BaseArray<f64, VectorContainer<f64>, 2>, 2> = get_matrix(&points);
    ...
    rsrs_algo.tree_cycle();

    apply_decomposition(&mut kernel_mat2, rsrs_algo.level_boxes);
